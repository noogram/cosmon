...
mod tests {
    ...
    fn a_captured_stdout_is_never_an_answerable_question() {
        // The harness running this assertion *is* the failing geometry: cargo
        // captures the test binary's stdout, and on a developer's terminal
        // stdin may well still be that terminal. Under the old
        // `stdin().is_terminal()` guard that combination said "ask" — into a
        // stream nobody reads. The conjunction must refuse it either way.
        assert!(!io::stdout().is_terminal(), "cargo captures test stdout");
        assert!(!can_ask_interactively());
    }
