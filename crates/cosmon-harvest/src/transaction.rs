...
use crate::paths::{
    default_state_dir, require_project_identity, resolve_config_from_context, tmux_socket_name,
};
use crate::HarvestContext as Context;
...
impl Args {
    ...
    ///
    /// This is the seam the previous unit left for the library effect, and
    /// the library effect now uses it: `cosmon_rpp_adapter`'s
    /// `LibraryHarvestEffect` builds an `Args` here and calls [`run`] — the
    /// same [`run`] `cs done` calls with the `Args` clap parsed. The argv
    /// builder
    /// [`cs_done_argv`](cosmon_core::harvest_door::HarvestOptions::cs_done_argv)
    /// survives for the operator-declared `harvest_cs_binary` alternative,
    /// and the round-trip falsifier that says the two agree survives with it.
    #[must_use]
...
///
/// The boundary answers a [`CosmonError`](cosmon_core::error::CosmonError)
/// carrying the operator-facing sentence, and that sentence is exactly right —
/// it says what to do next. What it lacked was a *label*. For the CLI that
/// meant exit 1 where every other harvest refusal has its own code; for the
/// route it meant an anonymous `500 harvest_failed` in the single most likely
/// production shape there is: a galaxy that armed `[harvest_authority]` and
/// has not yet been given a grant. A door that refuses for an unnamed reason
/// is the defect ADR-176 exists to prevent, and this is the last place one
/// was left.
///
/// `not_authorized` (exit 71) rather than a ninth name, because it is the same
/// verdict the door's decision half already answers for an unarmed galaxy:
/// *this harvest is not authorised*. The two differ in which half noticed, not
/// in what a caller does about it.
///
...
/// molecule to any commit. Because [`crate::lineage::completion_trailers`]
...
fn compute_teardown_plan(
    ...
    // Fleet worker state.
    let state_dir = ctx.config.clone().unwrap_or_else(default_state_dir);
...
fn resolve_base_branch_for(
    ...
    crate::base_branch::resolve(repo_root, persisted, configured_trunk)
}

/// The galaxy's `[project] trunk_branch` declaration, when it has one.
///
/// The config read lives here, at the command edge, so
/// [`crate::base_branch`] stays a pure function of its arguments. A
...
pub fn run(ctx: &Context, args: &Args) -> anyhow::Result<()> {
    ...
    crate::pilot_gesture::refuse_unleased_pilot_gesture(
        &state_dir,
        &mol_id,
        "cs done",
        &crate::pilot_gesture::process_env,
    )?;
    ...
    if args.if_completed {
        ...
        if mol.status != MoleculeStatus::Completed || mol.merged_at.is_some() || mol.archived {
            ...
            let reclaimed = if already_merged {
                find_repo_root(ctx).map_or_else(
                    |_| Vec::new(),
                    |root| {
    ...
    let socket = tmux_socket_name(ctx);
    ...
    // on.
    let resolved_base = crate::base_branch::resolve_with_source(
        &repo_root,
        mol.base_branch.as_deref(),
    ...
    //
    // `state_dir`, never a fresh walk-up. A walk-up answers "the galaxy
    // containing the current directory", which is the right answer for a CLI
    // standing in its own galaxy and a *dangerous* one for the library
    // caller: a server harvesting a tenant would append this molecule's merge
    // events to the events log of whatever galaxy the adapter happens to be
    // installed in, and then try to commit that foreign path. Observed the
            ...
            Ok(plans) if !plans.is_empty() => {
                for p in &plans {
                    actions.push(format!(
                        "renumbered_adr: {} → ADR-{}",
                        p.old_path,
                        crate::adr::format_adr_number(p.new_number)
                    ));
                }
            }
    ...
    let lineage_trailers = crate::lineage::completion_trailers(&store, &mol_id);
    ...
    if merge_succeeded {
        ...
        if let Some(ref hook_cmd) = cfg.hooks.post_merge {
            ...
            let trunk = crate::base_branch::reference_trunk(
                &repo_root,
                ...
                post_merge_deploy_allowed(&base_branch, &trunk, args.deploy_off_trunk)
            {
                ...
                // `Stdio::null()` stdout.
                let hook_ran = match crate::trust::ensure_trusted(&repo_root) {
...
fn try_merge_with_escalation(
    ...
    branch: &str,
    base: &str,
    base_source: crate::base_branch::BaseSource,
    strategy: MergeStrategy,
    session_name: &str,
    socket: &str,
    auto_propel: bool,
    ...
    for retry in 0..max_retries {
        ...
        if backend.is_alive(&wid).unwrap_or(false) {
            ...
            let _ = backend.send_input_observed(
                &wid,
                propel_msg,
                &crate::injection_provenance::propulsion(mol_id, &mol_state_dir),
            );
            std::thread::sleep(std::time::Duration::from_millis(300));
            let _ = backend.send_input_observed(
                &wid,
                "",
                &crate::injection_provenance::propulsion_submit(mol_id, &mol_state_dir),
            );
        } else if !ctx.json {
...
fn run_delegated_command(
    ...
    if let Err(e) = crate::trust::ensure_trusted(repo_root) {
    ...
    let (program, args) = match crate::egress_delegate::jail_delegated_sh(command) {
        crate::egress_delegate::JailDecision::Ready {
            program,
            args,
            advisory_reason,
            ..
        ...
        crate::egress_delegate::JailDecision::Refused { message } => {
            return Err(anyhow::anyhow!(
...
/// (the binary's own `BUILD_TREE` stamp), not the commit SHA. A commit SHA is a
...
/// trusting the running process's own the binary's own `BUILD_TREE` stamp: the
...
/// the base already owns, [`crate::adr::plan_renumber`] assigns the next
...
fn renumber_colliding_adrs(
    worktree_path: &Path,
    base_branch: &str,
) -> anyhow::Result<Vec<crate::adr::RenumberPlan>> {
    if !worktree_path.is_dir() {
        return Ok(Vec::new());
    }
    ...
    let base_numbers: Vec<u32> = String::from_utf8_lossy(&base_ls.stdout)
        .lines()
        .filter_map(crate::adr::parse_adr_number)
        .collect();
    ...
    let branch_added: Vec<String> = String::from_utf8_lossy(&diff.stdout)
        ...
        .filter(|p| crate::adr::parse_adr_number(p).is_some())
        .collect();

    let plans = crate::adr::plan_renumber(&base_numbers, &branch_added);
    if plans.is_empty() {
        return Ok(Vec::new());
    }
    ...
    for plan in plans {
        ...
        if let Ok(content) = std::fs::read_to_string(&abs_old) {
            let rewritten =
                crate::adr::rewrite_self_reference(&content, plan.old_number, plan.new_number);
        ...
        let msg = format!(
            ...
            crate::adr::format_adr_number(plan.old_number),
            crate::adr::format_adr_number(plan.new_number),
        );
        let commit = Command::new("git")
            .args([
                "-C",
...
///
/// Delegates to [`crate::target_repo::resolve`] — the galaxy's `[project]
...
fn find_repo_root(ctx: &Context) -> anyhow::Result<PathBuf> {
    crate::target_repo::resolve_from_config_in(
        &resolve_config_from_context(ctx),
...
mod tests {
    ...
    /// callers.
    ///
    /// Before the transaction became a library, every path out of
    /// [`crate::done_authority::authorize_harvest`] was an anonymous
    /// `CosmonError`: `cs done` exited 1 where every other harvest refusal has
    /// its own code, and the RPP route answered `500 harvest_failed`. That is
    /// the shape a galaxy is in the moment it arms `[harvest_authority]` and
    /// before an operator drops a grant — the most likely production state of
    /// all — and a door that refuses for an unnamed reason is what ADR-176
    /// exists to prevent.
    ///
    /// Goes red if the refusal loses its label (back to a bare `anyhow`), if
    /// it takes a different code, or if the ADR-172 §D5 claim bound is
    /// dropped from the text an operator reads.
    #[test]
    fn an_unauthorised_harvest_is_refused_by_name_and_keeps_its_claim_bound() {
        let err = name_the_authority_refusal(
            "no operator-sealed authorisation covers harvesting task-20260901-6da6 into \
             main.\n\nThis refuses an *unauthorised cosmon harvest*. It does not make the \
             trunk\nimmutable.\n"
                .to_owned(),
        );
        ...
        assert_eq!(
            ...
            Some(cosmon_core::harvest_door::DoorRefusal::NotAuthorized.exit_code()),
            "the CLI caller must branch on 71, not on the generic 1"
        );
        let refused = err
            .downcast_ref::<RefusedHarvest>()
            .expect("the refusal must be typed for the library caller too");
        assert_eq!(
            refused.refusal,
        ...
        assert!(
            text.contains("does not make the trunk"),
            "the ADR-172 D5 claim bound must survive into the named refusal: {text}"
        );
    }
    ...
    fn completion_merge_carries_ledger_derived_lineage_trailers() {
        ...
        let trailers = crate::lineage::completion_trailers(&store, &child);
    ...
    fn ff_only_single_molecule_refused_then_lineage_resolves_after_merge() {
        ...
        let lineage_trailers = crate::lineage::completion_trailers(&store, &mol);
        let merge_trailers: Vec<String> = coauthor_trailers
            .iter()
            .cloned()
            .chain(lineage_trailers.iter().cloned())
        ...
        let resolved = crate::lineage::trailer_value(body, crate::lineage::MOL_ID_KEY);
        assert_eq!(
            resolved.as_deref(),
            Some(mol.as_str()),
    ...
    fn reference_trunk_discovers_origin_head_when_unconfigured() {
        ...
        assert_eq!(
            crate::base_branch::reference_trunk(repo, None),
            "trunk",
    ...
    fn reference_trunk_config_beats_origin_head() {
        ...
        assert_eq!(
            crate::base_branch::reference_trunk(repo, Some("release/2.0")),
            "release/2.0",
    ...
    fn configured_trunk_beats_origin_head_for_the_merge_destination() {
        if std::env::var_os(crate::base_branch::BASE_BRANCH_ENV).is_some() {
    ...
    fn recorded_worktree_for_resolves_bound_worker_repo() {
        ...
        let resolved = crate::worktree::recorded_worktree_for(&store, &mol, &galaxy).unwrap();
        ...
        assert!(crate::worktree::recorded_worktree_for(&store, &orphan, &galaxy).is_none());
    }
