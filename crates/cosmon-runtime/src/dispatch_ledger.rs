...
//!   session, which is exactly the shape [`crate::orphan_scan`]
...
//!
//! Issue #54 / U5. The ledger was born binary-private inside `cosmon-cli`,
//! which made "record a dispatch" reachable only by shelling the `cs`
//! binary — the exact runtime dependency the ADR-080 §3.5 clause (e)
//! envelope encodes and issue #54 retires. The runtime crate is the
//! library home of dispatch execution (it already owns the resident loop
//! and [`crate::orphan_scan`], the ledger's reader), so the CLI now
//! re-exports this module rather than owning it.
...
use cosmon_state::{MoleculeData, StateStore, WorkerData};

/// Failure modes of the dispatch ledger.
///
/// Typed rather than stringly so the CLI and library callers can render the
/// refusal with their own vocabulary while tests can assert on structure.
#[derive(Debug, thiserror::Error)]
pub enum DispatchLedgerError {
    /// A state-store read, write, or lock failed.
    #[error("state store error: {0}")]
    State(#[from] CosmonError),

    /// The fixed `tackle` agent identifier could not be constructed.
    ...
    /// recorded dispatch with no `WorkerSpawned` on the wire is bit-for-bit
    /// the forensic signature of the six lost molecules this module exists
    /// to prevent.
    #[error("failed to record WorkerSpawned for {worker} at {path}: {source}")]
    WorkerSpawnedEmit {
        /// The worker whose spawn could not be recorded.
        worker: WorkerId,
        /// The event-log path the append failed against.
        path: PathBuf,
...
/// spawned.
///
/// Constructed only by [`commit_dispatch`], and required by the spawn seam
/// (the CLI's `spawn_and_prompt`, the library executor in
/// [`crate::tackle_exec`]). The token carries the identities it witnesses so
/// a caller cannot pair a commit for one molecule with a spawn for another;
/// [`Self::molecule`] and [`Self::worker`] expose them for that check.
#[derive(Debug, Clone)]
...
pub fn commit_dispatch(
    ...
    if let Err(e) = register_tackle_worker(
        store,
        record.worker,
...
///
/// Tackle workers are bound 1-to-1 to a session and a molecule. Registering
/// them in `fleet.json` lets `cs patrol`, `cs patrol --propel`, `cs resume`,
/// and `cs ensemble` see and manage them uniformly with spawn/deploy workers.
///
...
/// # Errors
///
/// Returns [`DispatchLedgerError`] when the fleet cannot be saved or the
/// `WorkerSpawned` event cannot be appended — the caller
/// ([`commit_dispatch`]) then undoes its own writes and refuses the
/// dispatch.
pub fn register_tackle_worker(
    store: &FileStore,
    wid: &WorkerId,
    ...
    // act, written on the near side of the process creation (see the module
    // docs). The timestamp therefore marks the moment cosmon committed to
    // the worker, not the moment the readiness pipeline finished with it —
    // which is the boundary the energy probe and the attribution cat-test
    // actually want, and the only one that exists before a crash can
    // swallow it. A dispatch that then fails to spawn emits
    // `WorkerSpawnRolledBack` and removes the fleet entry.
    //
    // We deliberately do NOT also emit a seed WorkerHeartbeat here: a
    // heartbeat means "the live process just proved it exists" (1 bit of
    // real entropy). Emitting one from the spawner impersonates liveness —
    // it produced the exact failure mode diagnosed in task-4046 (silent
    // exec failure, heartbeat still on the wire). The only legitimate
    // heartbeat emitters are the worker process itself and its bridge
    // (`cs heartbeat`).
    //
    // The store's own state root, not `resolve_state_dir(None)`. The ambient
    // resolver answers about the process's environment; every write above
    // this line went through `store`. In production they are the same
    // directory and in a test they are not, which meant the event landed in
    // a different tree than the fleet entry it describes.
    ...
    // recorded dispatch. `commit_dispatch` is the primary caller and it runs
    // before the spawn — its own contract is that "in every error case
    // nothing has been spawned" — so failing here costs a dispatch that has
    // not started and keeps the signature meaning what the forensics assume.
    ...
    .map_err(|e| DispatchLedgerError::WorkerSpawnedEmit {
        worker: wid.clone(),
        path: events_path,
        source: e,
    })?;
...
///
/// The mirror image of [`crate::orphan_scan`]'s orphan: that one
...
mod tests {
    ...
    ///    the spawn seam cannot be called without one; and
