===== routes-v1 =====
**41 routes `/v1/` gelées** — recomptées depuis le canon à chaque génération (`crates/cosmon-rpp-adapter/data/surface_events.txt`, cosmon). La colonne *Effet* est dérivée du scope requis (godel C5 : un scope distinct par effet coûteux ou irréversible, ADR-080 §6.5) — jamais éditée à la main.
...
| 13 | artifact | GET | `/v1/molecules/{id}/artifacts` | `cosmon:artifact:read` | adapter-only |  |
| 14 | artifact | GET | `/v1/molecules/{id}/artifacts/{token}` | `cosmon:artifact:read` | adapter-only |  |
| 15 | artifact | PUT | `/v1/molecules/{id}/artifacts/{token}` | `cosmon:artifact:write` | adapter-only |  |
| 16 | auth-claude | POST | `/v1/auth/claude/start` | — | adapter-only |  |
| 17 | auth-claude | POST | `/v1/auth/claude/email` | — | adapter-only |  |
| 18 | auth-claude | GET | `/v1/auth/claude/{session_id}` | — | adapter-only |  |
| 19 | auth-claude | DELETE | `/v1/auth/claude/{session_id}` | — | adapter-only |  |
| 20 | auth-claude | POST | `/v1/auth/claude/confirm` | — | adapter-only |  |
| 21 | observ. | GET | `/v1/auth/me` | — | adapter-only |  |
| 22 | observ. | GET | `/v1/events` | `cosmon:events:subscribe` | adapter-only |  |
| 23 | observ. | GET | `/v1/molecules/{id}/logs` | `cosmon:logs:subscribe` | adapter-only |  |
| 24 | observ. | GET | `/v1/quota` | `cosmon:molecule:read` | adapter-only |  |
| 25 | observ. | GET | `/v1/noyaux` | — | adapter-only |  |
| 26 | observ. | GET | `/v1/workers` | `cosmon:worker:read` | adapter-only |  |
| 27 | avatar-canal | POST | `/v1/avatar/converse` | `cosmon:pilote:converse` | tenant-verb |  |
| 28 | avatar-canal | POST | `/v1/avatar/perceive` | `cosmon:world:observe` | adapter-only |  |
| 29 | avatar-life | GET | `/v1/avatar/{instance_id}/status` | `cosmon:world:observe` | tenant-verb |  |
| 30 | avatar-life | POST | `/v1/avatar/{instance_id}/incarnate` | `cosmon:pilote:converse` | tenant-verb |  |
| 31 | avatar-life | POST | `/v1/avatar/{instance_id}/grant` | `cosmon:pilote:converse` | tenant-verb |  |
| 32 | avatar-life | GET | `/v1/avatar/{instance_id}/audit` | `cosmon:world:observe` | tenant-verb |  |
| 33 | avatar-life | GET | `/v1/avatar/{instance_id}/mould-info` | `cosmon:world:observe` | tenant-verb |  |
| 34 | admin | POST | `/v1/admin/habilitations` | — | adapter-only |  |
| 35 | admin | GET | `/v1/admin/habilitations` | — | adapter-only |  |
| 36 | admin | DELETE | `/v1/admin/habilitations/{id}` | — | adapter-only |  |
| 37 | admin | POST | `/v1/admin/reload` | — | adapter-only |  |
| 38 | admin | POST | `/v1/admin/federations` | — | adapter-only |  |
| 39 | admin | GET | `/v1/admin/federations` | — | adapter-only |  |
| 40 | admin | DELETE | `/v1/admin/federations/{id}` | — | adapter-only |  |
| 41 | admin | DELETE | `/v1/admin/federations/{id}/galaxies/{galaxy}` | — | adapter-only |  |

Découpage : **12** molecule + **3** artifact + **5** auth-claude + **6** observ. + **2** avatar-canal + **5** avatar-life + **8** admin = **41**.
...
Bijection liée : **16** (10 molecule + 1 avatar-canal + 5 avatar-life). Exemptes : **25**. Total : **41** — recompté depuis le canon (colonne `exposure`) à chaque génération.
