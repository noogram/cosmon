...
pub const LONG_ABOUT: &str = "Cosmon keeps a fleet of AI agents on track. Run several on one \
             ...
             presence ping|ls|gc|poll|send|inbox|lease'). 'send' and 'inbox' are the \
             ...
             cs presence lease — who may pilot a mission, and how the controls \
             change hands (ADR-168 D6). Authority is a lease with an epoch, and a \
             gesture carries the epoch it believes it holds; anything unknown is \
             read-only. 'lease show' prints the holder, the epoch and the \
             unanswered asks; 'lease request' lets a pilot ask and confers nothing \
             ...
             --epoch N' is refused unless the ledger agrees, and a pilot whose \
             lease moved is demoted by its own next heartbeat.\n\n\
