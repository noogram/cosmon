...
# history.
#
# EVERY identifier below is synthetic. The fixtures have to carry the shapes
# the gate detects, so the lines that do declare themselves with the per-line
# marker `publish: allow — <reason>` that `publish.sh --check` rule G honours —
# the same compensating control B and F use, and a waiver a reviewer reads in
# the diff rather than a pathspec exclusion nobody sees again.
#
...
printf 'Closes #1.\n\nhttps://claude.ai/code/session_fedcba9876543210\n' > "$TMP/body.md"  # publish: allow — synthetic fixture id; this harness is the gate's own test
...
#     scope, so the historical trailers on public main cannot red a build
#     that adds nothing.
