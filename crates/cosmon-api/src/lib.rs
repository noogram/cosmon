...
//!
//! CORS is [`CorsPolicy::Deny`] by default: a request carrying any
//! `Origin` is refused `403` before the handler runs, so a browser's
//! simple `POST` cannot spawn a worker and then be told off about it
//! afterwards. The native pilots send no `Origin` and are unaffected.
//! A wildcard on an unauthenticated executing surface would let any
//! page the operator visits drive this daemon; named origins are
//! opt-in via `--allow-web-origin`.
//!
...
///
/// The CORS middleware is installed for **both** policies. It used to
/// be installed only for [`CorsPolicy::Allow`], on the reasoning that
/// absent code cannot emit a header by accident — true, and beside the
/// point: the risk on this daemon is the write, not the header, and a
/// browser's simple `POST` reaches the handler whatever the response
/// says afterwards. Under [`CorsPolicy::Deny`] the middleware is what
/// turns a cross-origin write away before it happens.
