...
because a binary is missing, nor because an effect is unimplemented:
the door's decision half runs in-process and the suite ARMS it in the
throwaway galaxy, so the decision admits; and the effect half is a
library the adapter links (`cosmon-harvest`), so it RUNS. It refuses
`not_authorized` — for want of an ADR-172 operator seal, which nothing
that ships can mint. ADR-176 §12 as amended by issue #51 and #67.
"""
...
class TestTenantJourney:
    ...
    def test_done_returns_its_named_refusal(self, logged_in, molecule, worked, cfg, expect):
        ...
        `not_completed`, `reservation_requires_seal`, `backlog_full` —
        has been made inapplicable on purpose, so the only thing left to
        answer is the transaction itself. And the transaction RUNS here:
        ...
        because no operator seal covers this harvest — and this suite
        mints none, because nothing that ships can. That refusal is the
        CONTRACT here: it says the effect exists and re-derived its
        facts. A zero exit that integrated nothing is the defect issue
        #51 reported; a silent `cs` fallback is what U6 retired; and a
        `500 harvest_failed` here would mean the transaction lost the
        name of its own refusal.
        ...
        expect.truthy(
            rc != 0,
            "no operator seal covers this harvest and this suite mints none, so a "
            "zero exit here would mean the effect boundary let an unauthorised "
            "harvest through",
        )
        expect.equals(
            _refusal_label(payload, stderr),
            cfg.expect_done_label,
            "ADR-176 §12: the LIBRARY effect half runs in-process and refuses "
            "`not_authorized` for want of an ADR-172 operator seal. "
            "`harvest_effect_unavailable` here means this image lost the library "
            "harvest; `harvest_failed` means the transaction ran and lost the NAME of "
            "its refusal — set RPP_E2E_EXPECT_DONE_LABEL and update this expectation "
            "in the same commit",
        )
