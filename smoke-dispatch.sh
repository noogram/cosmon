...
# The fleet injects $MOLECULE_DIR. The by-hand fallback is derived by walking
# up out of the worktree rather than hardcoded: an absolute path naming one
# machine's home directory is exactly the kind of thing that must not be
# tracked, and it would be wrong on every other checkout anyway.
MOLECULE_ID="${MOLECULE_ID:-task-20260726-1f8e}"
