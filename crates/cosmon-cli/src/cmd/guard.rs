...
pub enum GuardError {
    ...
    /// ADR-168 §D6 — a lifecycle gesture on a co-piloted mission, from a
    /// session that does not hold its lease.
    ///
    /// Transparent over
    /// [`cosmon_harvest::pilot_gesture::UnleasedPilotGesture`]: the guard
    /// itself moved to `cosmon-harvest` when the harvest transaction did, so
    /// that the RPP route's library harvest passes the same gate the CLI
    /// does. This variant keeps the refusal inside the CLI's exit-code table
    /// without restating its message.
    #[error(transparent)]
    UnleasedPilotGesture(#[from] cosmon_harvest::pilot_gesture::UnleasedPilotGesture),
}
...
impl GuardError {
    ...
    pub fn exit_code(&self) -> i32 {
        match self {
            ...
            GuardError::UnleasedPilotGesture(_) => exit_code::UNLEASED_PILOT_GESTURE,
        }
    }
}
...
/// The pilot-lease authority guard, as the five lifecycle verbs reach it.
///
/// The decision and its message live in
/// [`cosmon_harvest::pilot_gesture::refuse_unleased_pilot_gesture`] — the
/// harvest is one of the five gestures, and the RPP route performs it without
/// going through this crate, so a copy here would be a second gate. This is
/// the CLI's re-export: it keeps the guard's refusal inside [`GuardError`],
/// which is what maps it to exit code
/// [`exit_code::UNLEASED_PILOT_GESTURE`].
///
/// # Errors
///
...
where
    F: Fn(&str) -> Option<String>,
{
    cosmon_harvest::pilot_gesture::refuse_unleased_pilot_gesture(
        state_root, mission, gesture, env_lookup,
    )
    .map_err(rehome_unleased_pilot_gesture)
}

/// Re-home a bare [`cosmon_harvest::pilot_gesture::UnleasedPilotGesture`] into
/// [`GuardError`], leaving every other error untouched.
///
/// The library refuses with its own type — it has no opinion about a CLI's
/// exit codes — and `main` reads the exit code off [`GuardError`]. Anything
/// that calls into `cosmon-harvest` and can hit the lease guard must pass its
/// error through here, or the refusal an operator reads as
/// [`exit_code::UNLEASED_PILOT_GESTURE`] arrives as a generic exit 1 with the
/// right words on it. `cs done` is the second such caller: the harvest runs
/// the guard inside the transaction, not through this module's wrapper.
#[must_use]
pub fn rehome_unleased_pilot_gesture(err: anyhow::Error) -> anyhow::Error {
    match err.downcast::<cosmon_harvest::pilot_gesture::UnleasedPilotGesture>() {
        Ok(refusal) => GuardError::from(refusal).into(),
        Err(other) => other,
    }
}

pub use cosmon_harvest::pilot_gesture::process_env;
