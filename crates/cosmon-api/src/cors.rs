...
impl CorsPolicy {
    ...
    /// Whether any browser origin is allowed. `router` uses this to
    /// decide whether to install the middleware at all.
    #[must_use]
...
///
/// Only ever installed when the policy allows at least one origin (see
/// [`crate::router_with_cors`]); under [`CorsPolicy::Deny`] there is no
/// middleware in the stack, so no header can be emitted by accident.
pub(crate) async fn layer(
    ...
    if req.method() == Method::OPTIONS {
        ...
        // An unknown origin gets a preflight with no allow headers,
        // which the browser reads as a refusal.
