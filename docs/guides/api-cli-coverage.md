...
| `cs sessions` | NO | — | Co-pilotage cockpit over *provider* sessions (mission co-pilotage M5, ADR-168): discover/list/show/attach/peers/send/inbox/checkpoint/drift/takeover. Every verb reads or writes local state — provider session logs under `$HOME`, the presence registry, the pilot mailbox, the checkpoint store and the lease ledger — for pilots sharing one host. A remote principal has no provider log to observe and no seat to take; `takeover grant` in particular is the operator gesture ADR-168 §D6 refuses to automate, so exposing it would hand a JWT bearer the authority the whole invariant exists to keep human. |
