...
if [[ "$HAS_SENDKEYS" -eq 1 && "$HAS_BYPASS" -eq 1 && "$HAS_HEADLESS" -eq 0 ]]; then
  ...
  VERDICT="RED"
  NOTE="Corroborated: the transport spawn helper builds 'claude --permission-mode <mode> --prompt '...'' (crates/cosmon-transport/src/claude.rs:142-147). The PRODUCTION tackle path (build_claude_command in cosmon-cli tackle) launches the interactive Claude TUI in a detached tmux session under --permission-mode bypassPermissions with the prompt delivered via tmux paste-buffer (not argv) — the exact bypassPermissions-as-root refusal + briefing-hang surface the tester hit. bypass=$HAS_BYPASS sendkeys=$HAS_SENDKEYS. The v1 probe's 'RED via bypass+sendkeys' was right by accident but false-matched tmux's own -p; the decisive argv signal is the real --prompt flag. Runtime root-refusal/hang portion INCONCLUSIVE: $RUNTIME_NOTE."
else
