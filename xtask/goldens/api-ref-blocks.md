===== routes-v1 =====
**40 routes `/v1/` gelées** — recomptées depuis le canon à chaque génération (`crates/cosmon-rpp-adapter/data/surface_events.txt`, cosmon). La colonne *Effet* est dérivée du scope requis (godel C5 : un scope distinct par effet coûteux ou irréversible, ADR-080 §6.5) — jamais éditée à la main.
...
| 15 | auth-claude | POST | `/v1/auth/claude/start` | — | adapter-only |  |
| 16 | auth-claude | POST | `/v1/auth/claude/email` | — | adapter-only |  |
| 17 | auth-claude | GET | `/v1/auth/claude/{session_id}` | — | adapter-only |  |
| 18 | auth-claude | DELETE | `/v1/auth/claude/{session_id}` | — | adapter-only |  |
| 19 | auth-claude | POST | `/v1/auth/claude/confirm` | — | adapter-only |  |
| 20 | observ. | GET | `/v1/auth/me` | — | adapter-only |  |
| 21 | observ. | GET | `/v1/events` | `cosmon:events:subscribe` | adapter-only |  |
| 22 | observ. | GET | `/v1/molecules/{id}/logs` | `cosmon:logs:subscribe` | adapter-only |  |
| 23 | observ. | GET | `/v1/quota` | `cosmon:molecule:read` | adapter-only |  |
| 24 | observ. | GET | `/v1/noyaux` | — | adapter-only |  |
| 25 | observ. | GET | `/v1/workers` | `cosmon:worker:read` | adapter-only |  |
| 26 | avatar-canal | POST | `/v1/avatar/converse` | `cosmon:pilote:converse` | tenant-verb |  |
| 27 | avatar-canal | POST | `/v1/avatar/perceive` | `cosmon:world:observe` | adapter-only |  |
| 28 | avatar-life | GET | `/v1/avatar/{instance_id}/status` | `cosmon:world:observe` | tenant-verb |  |
| 29 | avatar-life | POST | `/v1/avatar/{instance_id}/incarnate` | `cosmon:pilote:converse` | tenant-verb |  |
| 30 | avatar-life | POST | `/v1/avatar/{instance_id}/grant` | `cosmon:pilote:converse` | tenant-verb |  |
| 31 | avatar-life | GET | `/v1/avatar/{instance_id}/audit` | `cosmon:world:observe` | tenant-verb |  |
| 32 | avatar-life | GET | `/v1/avatar/{instance_id}/mould-info` | `cosmon:world:observe` | tenant-verb |  |
| 33 | admin | POST | `/v1/admin/habilitations` | — | adapter-only |  |
| 34 | admin | GET | `/v1/admin/habilitations` | — | adapter-only |  |
| 35 | admin | DELETE | `/v1/admin/habilitations/{id}` | — | adapter-only |  |
| 36 | admin | POST | `/v1/admin/reload` | — | adapter-only |  |
| 37 | admin | POST | `/v1/admin/federations` | — | adapter-only |  |
| 38 | admin | GET | `/v1/admin/federations` | — | adapter-only |  |
| 39 | admin | DELETE | `/v1/admin/federations/{id}` | — | adapter-only |  |
| 40 | admin | DELETE | `/v1/admin/federations/{id}/galaxies/{galaxy}` | — | adapter-only |  |

Découpage : **11** molecule + **3** artifact + **5** auth-claude + **6** observ. + **2** avatar-canal + **5** avatar-life + **8** admin = **40**.
...
Bijection liée : **16** (10 molecule + 1 avatar-canal + 5 avatar-life). Exemptes : **24**. Total : **40** — recompté depuis le canon (colonne `exposure`) à chaque génération.
