...
| `cs done` | **NO (NEVER)** | — | **Trunk-writing gesture** — one verb, two authorities: closure of the molecule's own lifecycle, and integration into the resolved base every future molecule inherits. What is withheld is not the name but the **degrees of freedom**: `--force`, `--strategy`, `--skip-pre-done-hook`, `--no-branch-delete`, `--propel-message` — a derogation requested by its beneficiary is not a derogation ([ADR-176](../adr/176-remote-harvest-authority-is-a-sealed-capability.md) D4). The successor ADR §5.2 required exists ([ADR-176](../adr/176-remote-harvest-authority-is-a-sealed-capability.md)); its clause 2 is **replaced** — the authority model is `DoneAuthorization::Delegated` ([ADR-172](../adr/172-done-authority-is-an-operator-sealed-capability.md) D2), **not** a `delegate_for` claim, which seals a property of the bearer and so cannot be refused when the base has moved. Remote harvest stays refused for want of a mechanism (ADR-176 §9), not by doctrine. ADR-080 §5.1–§5.2; ADR-077. |
| `cs land` | V1 | `POST /v1/molecules/:id/land` | **The harvest door** — the answer to issue #51 ([ADR-176](../adr/176-remote-harvest-authority-is-a-sealed-capability.md)). Closes one molecule and, where the second authority arises, integrates it into the resolved base. It is not `cs done` with fewer flags reachable by convention: the argument set is fixed at the type (`done::Args::sealed_door`), so no strategy, no `--force`, no hook waiver and no propel message exists on this path at all (D4), and auto-propel is disarmed by construction (D6). **Two proofs, two keys**: the JWT authenticates the requester, an operator-sealed `HarvestGrant` authorises the effect — the verb refuses `not_authorized` in any galaxy that has not armed `[harvest_authority] required`. Synchronous by decision: a 202 on a transaction that may integrate nothing is the defect #51 reports. Seven named refusals, mirrored to stable exit codes 70–76. **Library-direct decision half (issue #54 U3, ADR-176 §11)**: the route runs the door's body in-process via `cosmon_filestore::harvest_door` — the same library entry `cs land` executes — so the four pre-effect refusals and `already_landed` idempotence answer with no `cs` binary present; only the admissible effect (the sealed `cs done` transaction) still crosses the subprocess seam, which the adapter cut-over unit retires. |
| `cs stitch` | **NO (NEVER)** | — | Operator-only (ADR-110 single-writer-trunk). Merges a mission's DAG closure onto `main` in topological order under the trunk lock — the canonical trunk writer. Same NEVER class as `cs done`: a trunk-writing gesture, never exposed over the RPP. This row already names the **effect** rather than a gesture, which is the formulation [ADR-176](../adr/176-remote-harvest-authority-is-a-sealed-capability.md) D4 makes the rule for the whole closed list. ADR-080 §5.1. |
