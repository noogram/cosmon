...
| — | ADAPTER-ONLY | `GET /v1/vitals` | Per-tenant molecule-keyed fleet view (issue #78, ADR-180): one row per non-terminal molecule, with stored lifecycle status kept separate from observed worker health (`unassigned`, `live`, `orphaned`, `unknown`), plus declared operator waits and backlog age from the shared staleness arithmetic. It is not `cs peek`: no operator-wide sensorium or terminal raster crosses the tenant boundary. |
